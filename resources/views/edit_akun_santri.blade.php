        ...
        <div class="col-md-8 m-auto">
            <div class="card shadow mb-4 mt-4">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Edit Akun Santri</h5>
                </div>
                <div class="card-body">
                    @if (session('success'))
                    ...
                    @endif
                    <form action="{{ route('update_akun_santri', $dataSantri->id) }}" method="POST">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ $dataSantri->id }}">
                        <div class="mb-3">
                            <label for="name" class="form-label">Nama Lengkap</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                   id="name" name="name" value="{{ old('name', $dataSantri->name) }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control @error('email') is-invalid @enderror" 
                                   id="email" name="email" value="{{ old('email', $dataSantri->email) }}" required>
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
